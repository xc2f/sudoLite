...
Page({
  ...
  reset() {
    ...
    data.map((itemRow, idxRow) => {
      itemRow.map((item, idx) => {
        // 从右下角退回
        if (this.data.optimization) {
          item.successAnimation = this.basicAnimation(50, 0)
            .scale(0)
            .step()
            .export();
        } else {
          item.successAnimation = this.basicAnimation(
            50,
            (8 - idx + 8 - idxRow) * 50 - 50
          )
            .scale(0)
            .step()
            .export();
  ...
  handleGenerateSudoku() {
    ...
    if (
      !this.data.init ||
      this.data.complete ||
      this.data.toolTip.type === 'ready' ||
      this.data.toolTip.type === 'end' ||
      this.data.toolTip.type === 'drop'
    ) {
      this.generateSudoku();
    } else {
      wx.showModal({
  ...
  computeTime() {
    ...
    let time = Math.round(
      ((this.pauseTime || new Date().getTime()) - this.startTime) / 1000
    );
  ...
  toggleDrawerHandler(type) {
    ...
    this.setData({
      ...
      menuAnimationTop: this.basicAnimation()
        .rotate(-menuRotate)
        .width(w)
        .step()
        .export(),
      menuAnimationMiddle: this.basicAnimation()
        .width(menuWidth)
        .step()
        .export(),
      menuAnimationBottom: this.basicAnimation()
        .rotate(menuRotate)
        .width(w)
        .step()
        .export(),
  ...
  togglePanel(toShow) {
    if (this.data.optimization) {
      ...
      this.setData({
        panelShowAnimation: this.basicAnimation(200)
          .scale(scale)
          .step()
          .export(),
      });
  ...
  isComplete(data, init = false) {
    ...
    let unComplete = data.some((itemRow) =>
      itemRow.some((item) =>
        !item.show ? (item.fill ? item.fill !== item.value : true) : false
      )
    );
  ...
  panelTap(e) {
    ...
    data.map((rowItem, rowIdx) => {
      rowItem.map((item, idx) => {
        ...
        if (item.show) {
          if (
            (rowIdx === boxCoords.y || idx === boxCoords.x) &&
            !(rowIdx === boxCoords.y && idx === boxCoords.x)
          ) {
  ...
  drawCanvas(showTime) {
    ...
    let cw = this.data.deviceInfo.windowWidth * 0.7;
    ...
    ctx.restore();
    ctx.fillText(
      '遮挡比例： ' + this.percentDegree,
      padding * 2,
      padding + lineHeight * 1 - lineHeight * 0.5
    );
    ctx.fillText(
      '等级： ' + this.degreeTitle,
      padding * 2,
      padding + lineHeight * 2 - lineHeight * 0.5
    );
    ctx.fillText(
      '用时： ' + showTime,
      padding * 2,
      padding + lineHeight * 3 - lineHeight * 0.5
    );
    ctx.fillText(
      '完成时间： ' + parseTime(new Date().getTime()),
      padding * 2,
      padding + lineHeight * 4 - lineHeight * 0.5
    );
    ctx.save();
    ctx.beginPath();
    ...
    // 6为字体大小的一半
    ctx.moveTo(padding, padding + lineHeight * 1 - lineHeight * 0.5 - 6);
    ctx.lineTo(padding, padding + lineHeight * 4 - lineHeight * 0.5 + 6);
    ctx.stroke();
    ctx.restore();
    ctx.draw();
  ...
  onShareAppMessage: function (e) {
    ...
    if (e.from === 'button') {
      ...
      title =
        (share && share.range[range]) ||
        '我在sudoLite完成了一项数独挑战，成绩如下：';
      // console.log(img)
    } else {
